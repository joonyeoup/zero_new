...
do_smoke() {
    ...
    echo "   SCREENSHOT_OUTPUT must be the SAME path as [mcp.servers.env].SCREENSHOT_OUTPUT"
    echo "   in config.toml — that is the PNG the sidecar serves back to the app on"
    echo "   GET /screenshot, which is how the TV shows what the agent captured."
    echo "   sdb shell 'GATEWAY_URL=http://127.0.0.1:42617 POSTPROCESS_PORT=8787 \\"
    echo "              SCREENSHOT_OUTPUT=<SCREENSHOT_OUTPUT_PATH> \\"
    echo "              $TV_HOME/analyze-screen-postprocess &'"
    echo "-- 3b. confirm the sidecar can see the capture (after one analyze run):"
    echo "   sdb shell 'curl -s http://127.0.0.1:8787/screenshot-info'   # {\"exists\":true,...}"
