...
do_smoke() {
    ...
    echo "   SCREENSHOT_OUTPUT must match [mcp.servers.env].SCREENSHOT_OUTPUT in config.toml —"
    echo "   that PNG is what the sidecar serves on GET /screenshot for the TV app to display."
    echo "   $SSH_CMD $TV_USER@$TV_IP 'GATEWAY_URL=http://127.0.0.1:42617 POSTPROCESS_PORT=8787 SCREENSHOT_OUTPUT=<SCREENSHOT_OUTPUT_PATH> $TV_HOME/analyze-screen-postprocess &'"
    echo ""
    echo "-- 5b. Confirm the sidecar sees the capture (after one analyze run):"
    echo "   $SSH_CMD $TV_USER@$TV_IP 'curl -s http://127.0.0.1:8787/screenshot-info'"
    echo ""
